...
\begin{proof}
  ...
  Let $q_i = \left\lfloor \dfrac{a_{i - 1}}{a_{i}} \right\rfloor$,
...
(ignoring the coefficient $k$ in Miller-Rabin test),
...
Pollard's Rho Algorithm\cite{lenstra1987} is based on Birthday Paradox,
...
The time complexity of ECM Algorithm is $L_p\left[\dfrac{1}{2}, \sqrt 2\right]$, where
$$
...
GNFS Algorithm\cite{nfs1993} is based on the bifield mapping
between the algebraic number field and the rational number field.
The time complexity of GNFS is $L_n \left[\dfrac{1}{3}, \left( \dfrac{64}{9} \right)^{\frac{1}{3}}\right]$.
